...
namespace SafriSoftv1._3.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            try
            {
                var usernameClaim = UserManager.GetClaims(userData.Id).First(x => x.Type == "AccountLocked");
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    Claim OrganisationClaim = new Claim("Organisation", model.OrganisationName);
                    var saveClaim = await UserManager.AddClaimAsync(user.Id, OrganisationClaim);
                    Claim UsernameClaim = new Claim("Username", model.OrganisationName);
                    var saveUsernameClaim = await UserManager.AddClaimAsync(user.Id, UsernameClaim);
                    var saveRole = UserManager.AddToRole(user.Id, "Admin");
